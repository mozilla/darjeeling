define('log', ['storage', 'utils'], function(storage, utils) {
  ...
  var rawPersistentLogs = storage.getItem('persistentLogs');
  var persistentLogs = rawPersistentLogs ? JSON.parse(rawPersistentLogs) : {};
  ...
  logger.get_recent = function(count, type) {
    var selected_logs = type ? logs[type] : allLogs;
  ...
  logger.persistent = function(type) {
    ...
    args[2] = function(log_args) {
      if (!(type in persistentLogs)) {
        persistentLogs[type] = [];
      }
      persistentLogs[type].push(log_args);
      storage.setItem('persistentLogs', JSON.stringify(persistentLogs));
    };
    return logger.apply(this, args);
  };
  logger.persistent.all = persistentLogs;
